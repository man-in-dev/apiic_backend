...
const IncubationApplication = require('../models/IncubationApplication');
const { auth, adminAuth } = require('../middleware/auth');
const { incubationApplicationSchema, updateIncubationApplicationSchema, queryParamsSchema } = require('../validation/incubationValidation');

const router = express.Router();

// @route   POST /api/incubation
// @desc    Create a new incubation application
// @access  Private
router.post('/', async (req, res) => {
    try {
        // const { error, value } = incubationApplicationSchema.validate(req.body);
        // if (error) {
...
// @desc    Get all incubation applications with filtering and pagination
    ...
    try {
        ...
        if (search) {
            filter.$or = [
                ...
                { innovationDescription: { $regex: search, $options: 'i' } }
            ];
        }
        ...
        const applications = await IncubationApplication.find(filter)
            .sort(sort)
            .skip(skip)
            .limit(limit);

        const total = await IncubationApplication.countDocuments(filter);
        ...
        console.error('Get incubation applications error:', error);
        res.status(500).json({
            success: false,
...
// @desc    Get a single incubation application
// @access  Private
router.get('/:id', auth, async (req, res) => {
    try {
        ...
        console.error('Get incubation application error:', error);
        res.status(500).json({
            success: false,
...
// @desc    Update an incubation application
// @access  Private
router.put('/:id', auth, async (req, res) => {
    try {
        const { error, value } = updateIncubationApplicationSchema.validate(req.body);
        if (error) {
            return res.status(400).json({
                success: false,
                message: 'Validation error',
        ...
        const updatedApplication = await IncubationApplication.findByIdAndUpdate(
            req.params.id,
            value,
        ...
        res.json({
            success: true,
            message: 'Incubation application updated successfully',
            data: { application: updatedApplication }
        });
    } catch (error) {
        console.error('Update incubation application error:', error);
        res.status(500).json({
            success: false,
...
// @desc    Delete an incubation application
...
router.delete('/:id', auth, adminAuth, async (req, res) => {
    try {
        ...
        await IncubationApplication.findByIdAndDelete(req.params.id);

        res.json({
            success: true,
            message: 'Incubation application deleted successfully'
        });
    } catch (error) {
        console.error('Delete incubation application error:', error);
        res.status(500).json({
            success: false,
...
// @route   GET /api/incubation/stats/overview
// @desc    Get incubation application statistics
// @access  Private (Admin/Reviewer)
router.get('/stats/overview', auth, async (req, res) => {
    try {
        const stats = await IncubationApplication.aggregate([
            {
                $group: {
                    _id: null,
                    total: { $sum: 1 },
        ...
        console.error('Get incubation stats error:', error);
        res.status(500).json({
            success: false,
